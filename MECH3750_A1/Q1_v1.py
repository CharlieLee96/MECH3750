...
def sub_num(func,t,u,v,w):
    ...
    wn = float(w)
    if type(func) == 'Add':
        print(func.subs({'t':tn,'u':un,'v':vn,'w':wn}))
        mat.append(func.subs({'t':tn,'u':un,'v':vn,'w':wn}))
    if type(func) == 'list':
        for i in func:
            if i == 0:
                mat.append(0)
...
df1n = sub_num(df1,x0[0],x0[1],x0[2],x0[3])
df2n = sub_num(df2,x0[0],x0[1],x0[2],x0[3])
df3n = sub_num(df3,x0[0],x0[1],x0[2],x0[3])
df4n = sub_num(df4,x0[0],x0[1],x0[2],x0[3])            
Jn = sym.Matrix([df1n,df2n,df3n,df4n]) #Jacobian matrix with numerical value
print("Jn =   ",Jn)    

f1n = sub_num(f1,x0[0],x0[1],x0[2],x0[3])
f2n = sub_num(f2,x0[0],x0[1],x0[2],x0[3])
f3n = sub_num(f3,x0[0],x0[1],x0[2],x0[3])
f4n = sub_num(f4,x0[0],x0[1],x0[2],x0[3])   
print("f1n = ",f1n)
